...
void main()
{
	...
	*/
	...
	for (auto & p : fs::directory_iterator(templateGestureSetPath))
	{
		...
		{
			tg = M::GestureParser::dump_to_stdout(fs::path(p).string().c_str(), M::Gesture::eGestureType::Template, false);
			...
			cout << recognizer.gestureToString(tg) << endl;
...
/*
	...
	M::Gesture tg = M::GestureParser::dump_to_stdout("GestureSet/TemplateGestures/B3.xml" , M::Gesture::eGestureType::Template, false);

	M::PointMap pm = tg.getPointMap();
	M::CircGesture circgesture = M::CircGesture(pm, "A3", M::Gesture::eGestureType::Template);


	// Print CircGesture Info

	cout << "Num of anchors = " << circgesture.getNumOfAnchors() << endl;
	cout << circgesture.ToString() << endl;
...
*/
	...
	for (auto & p : fs::directory_iterator(candidateGestureSetPath))
	{
		if (fs::path(p).extension() == ".xml")
		{
			classification = recognizer.Classify( M::GestureParser::dump_to_stdout(fs::path(p).string().c_str(), M::Gesture::eGestureType::Candidate, false) ) ;
