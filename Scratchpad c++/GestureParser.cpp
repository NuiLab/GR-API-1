...
void GestureParser::parseGestureAttributes(TiXmlElement* pElement, unsigned int indent, const TiXmlString elementName)
{
	if (!pElement) return;
...
void GestureParser::parseGesture(TiXmlNode* pParent, unsigned int indent = 0)
{
	if (!pParent) return;
	...
	case TiXmlNode::TINYXML_ELEMENT:
		...
		{
			parseGestureAttributes(pParent->ToElement(), indent + 1, pParent->ValueTStr());
		}
	...
	{
		parseGesture(pChild, indent + 1);
	}
}

// load the named file and dump its structure to STDOUT
Gesture GestureParser::parseGesture(const char* pFilename, Gesture::eGestureType gestureType, bool verboseFlag = false)
{
	verbose = verboseFlag;
