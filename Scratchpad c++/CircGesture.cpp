...
void CircGesture::UpdateEventsMaps(map<CircGesture::Directions, vector<double>> eventMap, CircGesture::Directions direction, double observation)
{
	...
	{
		eventMap.at(direction).push_back(observation);		// VERIFY THIS IS AN ACCURATE APPROACH MAY BE CAUSE OF INACCURACY!!!
	}
	else
	{
...
CircGesture::DirectionalEvents  CircGesture::DirectionalEvents::CreateDirectionalEvents(vector<double> traceObservations)		// MAY BE THE CAUSE FOR INACCURACY
{
...
void CircGesture::DirectionalEvents::LogDirection(CircGesture::Directions direction, double observation)			// FIX ALL MAP INSERTIONS!!!! CAUSE OF INACCURACY
{
	if (observations.find(direction) != observations.end())				
